...
/// The needle map behind `HomeSurfacingTests`, split off at the
/// §2.1 ceiling (#1365) — one file per concern, the test in the
/// suite's own file and the register here.
extension HomeSurfacingTests {
    ...
    static let needles: [String: [String]] = [
        "Settings/SettingsView.swift": [
            ...
            // comes forward from System Settings. Either line
            // alone leaves a card that never updates.
            ".onAppear{model.refreshMacChecklist()}",
            ".onReceive(NotificationCenter.default.publisher("
                + "for:NSApplication.didBecomeActiveNotification"
                + ")){_inmodel.refreshMacChecklist()}",
        ],
