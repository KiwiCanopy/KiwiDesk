...
struct SpaceBarBadgeTests {
    ...
    /// #1214: each display's bar used to ask the render question
    /// with its OWN shown space, so a ∞ window was listed under
    /// the current Space of every screen at once. The verdict is
    /// the layout's — one active Space, one glyph.
    ...
    func globalStickyStaysOnTheActiveScreen() throws {
        ...
        // Active Space 1 lives on `display`; the other screen
        // shows Space 3, which is not focused, so its item lists
        // its own window and nothing else.
        let away = try #require(
            core.spaceBarItems(display: other, style: style)
        ...
        // Focus the other screen: the one glyph crosses with the
        // user — injected at its home index, so ahead of Note —
        // and leaves the screen it came from.
        core.state.workspaces.activate(SpaceID("3"))
