...
extension EventLoop {
    ...
    /// The carried arm (#1145). A window sticky reach has IN
    /// FLIGHT is EXPECTED present after a Desktop switch — the
    /// bridge MOVE puts it on the arriving Desktop — but for the
    /// transition's beat it is on NO reading: its AX element dies
    /// as it leaves the visible Space and the census has not
    /// composited it yet. So its removal is refused while the
    /// carry holds it (`carriedRemovalArmIsOpen`) — outright while
    /// the census shows it (#1157's own rule), and census-blind
    /// while the ONE episode ledger still has arms
    /// (`removalRecheckCap`), so every blind refusal is followed
    /// by the recheck that can end it. State AND
