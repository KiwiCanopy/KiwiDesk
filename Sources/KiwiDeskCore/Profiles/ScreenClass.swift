...
/// do. The five cases are disjoint only because they are tested
...
public enum ScreenClass: String, Sendable, CaseIterable, Codable {
    /// Taller than wide — a rotated ultrawide included (#1662).
    case pivoted
    /// 32:9 and wider: the aspect past `superUltrawideAspect`.
    case superUltrawide
    ...
    /// Aspect ratio threshold for the super-ultrawide class (3.0):
    /// 32:9 (3.56) above it, 21:9 (2.39) below (#1662).
    public static let superUltrawideAspect: CGFloat = 3.0
    ...
    public static func of(_ size: CGSize) -> ScreenClass {
        ...
        if size.width / size.height >= superUltrawideAspect {
