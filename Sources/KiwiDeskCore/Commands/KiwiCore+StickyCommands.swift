...
///
/// Applies unconditionally, and no surface may clamp the mark
/// back (Lua is open; `dim_factor` precedent). The GUI toggle
/// that once did is held ungated by `StickyMarkUngatedTests`,
/// which this module cannot see for itself.
extension KiwiCore {
    func stickyCommand(
        _ command: String,
        _ args: [JSONValue]
        ...
        case "desktop_reach":
            ...
            // Both directions take effect now: ON asserts, OFF
            // retires through the same reconcile (#1145).
            refreshStickyReach()
            return .ok()
        case "color":
