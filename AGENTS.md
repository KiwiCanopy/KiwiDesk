...
| `Keys`, `Events`, `Animation` | [input-and-animation.md](.claude/rules/input-and-animation.md) | Carbon hotkeys (no Input Monitoring permission), one `DisplayLink` per monitor; the spring integrator must stay inside its stability bound — a diverged animation never settles and kills the settle signal for the whole session (#599); a shrink snaps on frame 1 unless the caller marked it `SizeIntent.resize` (#593) — opt-in, never inferred, and the guard's `allowed` map is the one copy of who may |
