...
| `site/**` | [site.md](.claude/rules/site.md) | `{/* */}` not `<!-- -->` — template comments ship to visitors (#557); `site/.nvmrc` is the one copy of the Node version; `src/pages/404.astro` and `disable404Route` move together and the AA green-text literal carries one value across both stylesheets (#635) |
