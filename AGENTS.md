...
| `Keys`, `Events`, `Animation` | [input-and-animation.md](.claude/rules/input-and-animation.md) | Carbon hotkeys (no Input Monitoring permission), one `DisplayLink` per monitor, and the spring integrator must stay inside its stability bound — a diverged animation never settles and kills the settle signal for the whole session (#599) |
