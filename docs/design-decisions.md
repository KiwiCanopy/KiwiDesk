...
| If a future macOS keeps the private `SLSSetWindowLevel` symbol but silently ignores the level write, floating windows are *neither* pinned above the tiled plane *nor* re-raised on focus — the symbol's mere presence disables the AX fallback, which is strictly worse than fallback-only. | The level write is fire-and-forget (§5 / #357): its `CGError` is junk, so "the call worked" can't be observed at the call site. Gating the fallback on *demonstrated* effect would need a per-window level read-back after every promote — a SkyLight round-trip on the hot retile path — to defend against a macOS that does not exist. Every shipping macOS is present-and-honored. | `WindowLevel.isAvailable` reports `dlsym` symbol resolution, and the fast/fallback switch keys on it (`enforceFloatLevels`, `raiseFloatsAndSticky`, `raiseFloatsIfFallback`); the write's success is deliberately never read ([#418](https://github.com/hajiboy95/KiwiDesk/issues/418)). | None until such a macOS ships; the fix would re-read the level after promotion and revert to the AX re-raise on mismatch. On-device QA confirms a float stays above a focused full-frame tiled window today. |
