...
| During a scrolling focus slide, keystrokes still reach the previously focused app until the pan settles (one animation length, 50–1000 ms) — including the focus handoff after closing a window. In rare fast key-repeat across slow-AX apps (Electron), the late echo of an earlier raise can additionally revert the newest focus step. | Raising the target first pops a top-pinned row over the whole screen before the slide starts ([#143](https://github.com/hajiboy95/KiwiDesk/issues/143)); deferring the raise until the pan lands is what makes the pinned rows read as a real scroll. Normal for scroll-style window managers. | AppKit keyboard status only moves with the real AX raise, and the raise waits on the animation-settle signal (shared with the z-order restore). AX focus echoes carry no provenance, so a late self-echo is indistinguishable from a user click. | Global Carbon hotkeys are unaffected (they reach KiwiDesk regardless of the key app); `animations.set_on_scrolling(false)` disables the slide and restores instant transfer. Echo provenance tracking is the planned fix for the key-repeat revert ([#152](https://github.com/hajiboy95/KiwiDesk/issues/152)). |
