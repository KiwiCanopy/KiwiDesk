...
| The focus ring / drag ghost can be low-contrast against window content that happens to share its teal hue (a teal-branded app, a cyan terminal theme). | Overlays paint over arbitrary content, so no *static* hex can guarantee contrast against every possible window; `#0F8FAE` clears 3:1 on near-white and near-black (the common cases) and the ring's 2px stroke lets shape aid legibility even where hue contrast is weak. | The overlay color is a fixed value with no per-window content sampling — dynamic contrast would need live sampling of the covered pixels, deliberately out of scope for the brand convergence ([#439](https://github.com/hajiboy95/KiwiDesk/issues/439)). | Recolor the ring/ghost (`border.set_focused_color`, `drag.set_ghost_border_color`) if your work lives in same-hue apps; a content-sampling signal is a possible future spike. |
...
- **Drag ghost / drop-zone:** a deliberate two-hue split
  ...
  target. The two hues are the **content-overlay signal** below,
  not the brand green — see the carve-out.
...
window content. The brand kiwi green is a fill-only color
(`#AACB5D` ≈ 1.5:1 on white — fails AA) and vanishes as a thin
signal on light windows, so overlays use a contrast-tuned teal
`#0F8FAE` (a darker, more saturated cousin of the brand's border
teal `#89B3B5`, clearing 3:1 on both near-white and near-black);
the drag drop-zone keeps a distinct hue as a darkened amber
`#C2790A` (the old `#E8A33D` had the same light-window problem).
Bars keep the brand green because they own their backdrop; the
ring/ghost do not, so they trade brand fidelity for legibility.
This is the intended reading of the drag two-hue rule above —
