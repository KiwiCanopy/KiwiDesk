...
| A **square** focus border on a *rounded* window overlaps a little more window content near the edges (≈ 0.3× the corner radius) than a rounded border's 1 pt. | Square has no radius to match the window, so a shallow stroke leaves an empty corner between the sharp frame and the window's curve. Tucking the stroke's inner edge to the corner tangent (`R·(1−√2/2)`) makes the stroke band paint over that gap — a uniform inset, so the extra overlap shows on the straight edges too. The alternative (a non-uniform, corner-only inset) would need a custom per-corner path the design deliberately omits. This **capped-inner** model was briefly replaced by pure outset ([#303](https://github.com/hajiboy95/KiwiDesk/issues/303)) and then restored ([#311](https://github.com/hajiboy95/KiwiDesk/issues/311)): outset never touches content but leaves the ring reading *detached* — a hairline seam on rounded, a visible corner reveal on square — so a small guaranteed overlap (the seam/attachment concern) is a distinct reason from the neighbour-bleed row below and both are kept. | `BorderGeometry` deepens only the square inner overlap to the tangent point; the rounded path is unchanged, and it stays one stroke routine ([#278](https://github.com/hajiboy95/KiwiDesk/issues/278)). | Pick **Rounded** corners (the default) for the minimal 1 pt overlap; Square trades a little edge coverage for sharp corners with no empty gap. |
| A **thick** focus border at **small gaps** makes neighbouring windows' rings touch or visually merge, and at 0 pt gaps they overlap. | The ring is a pure post-layout overlay with no gap coupling (coupling would leak a visual setting into pure layout math). Its outward reach is `width − 1` per side for a rounded border (capped-inner keeps ≥ 1 pt of the stroke on the window), so rings meet when `2·(width−1) ≥ gap`; a square border tucks deeper (`width − R·(1−√2/2)`) and so reaches less and merges later, making rounded the worst case for this bound. The failure mode is a soft visual merge, never window-content clipping. An automatic minimum-gap floor was rejected: silently widening a user's `gap = 0` the moment borders turn on is paternalistic and changes a value the user set; `border.fit_gaps` stays the opt-in escape hatch. | The border is rendered independently of layout ([#278](https://github.com/hajiboy95/KiwiDesk/issues/278)); the width is bounded to 20 pt, but gaps and border width are set separately. | Set gaps at least as wide as the border (or run `border.fit_gaps`), or lower the width; the default 2 pt border is imperceptible at any typical gap. |
