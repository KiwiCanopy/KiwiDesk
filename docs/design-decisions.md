...
| Tiling is per-display: each connected monitor shows its own space at once, and moving a window to / focusing a space on another monitor no longer hides the first. A few secondary paths still resolve against the **main** screen — keyboard resize / float-nudge clamps, and some border geometry — so those operations on a *secondary* monitor can clamp to the wrong bounds. Focusing an **empty desktop** (no window) on another monitor also does not yet move the "focused display", so a new window opened right after can land on the previous monitor's space — and a **global sticky** window (which renders on the focused display) only "jumps" to the other monitor once the focused display actually moves, i.e. after you click a *window* there or switch its space, not on a bare empty-desktop click. | The active-space model is now per-display (`activeSpace(on:)` / `visibleSpaces`), which fixes the reported cross-monitor drag / move / focus / new-window bugs; the remaining `NSScreen.main` slot-math sites and the empty-desktop focus follow were scoped as a separate hardening pass to keep the state change contained. | `TilingEngine`/`stashInactive` are display-aware; `KiwiCore+Resize`, `KiwiCore+MouseResizeEnd`, `KiwiCore+FloatNudge`, and `BorderManager` still pick `NSScreen.main`, and there is no empty-desktop focus observer (contrast AeroSpace's `GlobalObserver`). | Multi-monitor slot-math hardening + empty-desktop focus follow (roadmap `plan/06_Roadmap.md`, plan `plan/multi-monitor-per-display-active-space.md`). |
