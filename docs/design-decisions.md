...
| A window KiwiDesk *ignores* — Ghostty's quick terminal, its own Settings window, or any future user `ignore_rules` app — can sit under a **top** app bar with its title bar (grab handle) hidden, and is never pushed clear the way a tracked floating window is. The bar being invisible over it is the visible symptom. | The top-bar clamp reaches only windows KiwiDesk *tracks*; an ignored window is deliberately outside management entirely — no tracking, no events, no frame assertion — so there is no seam at which to correct it. Clamping it would mean tracking it, defeating the point of ignoring. User-configurable ignore rules ([#176](https://github.com/hajiboy95/KiwiDesk/issues/176)) widen the set of windows this applies to. | Ignored windows are filtered before any state or layout (`FloatDetection.shouldIgnore`, and KiwiDesk's own process via `EventLoop.isOwnProcess`, [#174](https://github.com/hajiboy95/KiwiDesk/issues/174)); the clamp (`clampFloatsBelowTopBars`) runs over tracked floats only ([#242](https://github.com/hajiboy95/KiwiDesk/issues/242)). | Move the window by hand, or don't run a top-edge bar over it. KiwiDesk's own Settings window becomes clamp-covered once [#177](https://github.com/hajiboy95/KiwiDesk/issues/177) *floats* it (tracked) rather than fully ignoring it. |
...
| An app with **no bundle identifier** (a rare unbundled or legacy helper process) can't be targeted by an app rule (`float_rules`, `app_rules`) or by `pull_or_spawn`; its windows match no rule and tile normally. | App rules key on the bundle identifier because it is the only stable, locale- and rename-proof identity — the display name is presentation, not identity. A name-based fallback for id-less apps would reintroduce exactly the ambiguity (two apps sharing a display name, locale drift) that keying on the id removes, and split the vocabulary into two identity schemes. | Window identity is `NSRunningApplication.bundleIdentifier`, captured once at attach and stored on `ManagedWindow.appBundleID` (`AppRef`); a nil id stays nil and matches no rule ([#262](https://github.com/hajiboy95/KiwiDesk/issues/262)). | None — effectively every user-facing app is bundled and has an identifier; the blind spot is a handful of window-less helper processes not worth managing. |
| A **square** focus border on a *rounded* window leaves a small reveal at each corner — the square stroke's corner extends past the window's curve into the background. | It is the intended look, not a defect: a squared frame around a rounded photo. Detecting and flushing the stroke to the window's true silhouette would add per-corner masking logic the design deliberately omits, and Square exists precisely to *ignore* the real radius. | The focus border draws one stroke path with corner radius = real window radius (rounded) or `0` (square) — one geometry routine, no per-window corner shaping ([#278](https://github.com/hajiboy95/KiwiDesk/issues/278)). | Pick **Rounded** corners (the default) to hug the window; Square is for people who want sharp corners or already-square windows. |
| A **thick** focus border at **small gaps** makes neighbouring windows' rings touch or visually merge, and at 0 pt gaps they overlap. | The ring is a pure post-layout overlay with no gap coupling (coupling would leak a visual setting into pure layout math). Its outward reach is `width − 1` per side, so rings meet when `2·(width−1) ≥ gap`; the failure mode is a soft visual merge, never window-content clipping (inner overlap is pinned at 1 pt). | The border is rendered independently of layout ([#278](https://github.com/hajiboy95/KiwiDesk/issues/278)); the width is bounded to 20 pt, but gaps and border width are set separately. | Set gaps at least as wide as the border, or lower the width; the default 2 pt border is imperceptible at any typical gap. |
...
every overflow pile to a single ring on its top window via
`Navigation.pileMates` (buried mates are excluded), and in monocle
— where only the focused window is visible — it stays focused-only
regardless of `border.unfocused_enabled`. Because it dedupes by
pile geometry rather than layout identity, it needs no per-layout
branch: `KiwiCore.borderSpecs` is pure over the flat slot list.
