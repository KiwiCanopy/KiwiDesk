...
| Tiling is per-display: each connected monitor shows its own space at once, and moving a window to / focusing a space on another monitor no longer hides the first. A few secondary paths still resolve against the **main** screen — keyboard resize / float-nudge clamps, and some border geometry — so those operations on a *secondary* monitor can clamp to the wrong bounds. | The active-space model is per-display (`activeSpace(on:)` / `visibleSpaces`), which fixes the reported cross-monitor drag / move / focus / new-window bugs. Empty-desktop focus follow is now enforced ([#446](https://github.com/hajiboy95/KiwiDesk/issues/446)): a bare left click on another monitor moves the focused display, so a new window / global sticky follow the cursor. The remaining `NSScreen.main` slot-math sites were scoped as a separate hardening pass to keep the state change contained. | `TilingEngine`/`stashInactive` are display-aware, and `MouseTracker.onLeftMouseDown` → `followDisplayUnderClick` moves the focused display on a bare click; `KiwiCore+Resize`, `KiwiCore+MouseResizeEnd`, `KiwiCore+FloatNudge`, and `BorderManager` still pick `NSScreen.main`. | Multi-monitor slot-math hardening (roadmap `plan/06_Roadmap.md`, plan `plan/multi-monitor-per-display-active-space.md`). |
| Moving the only window off the focused display's space (without follow) hands key focus to the **desktop by activating Finder**. When Finder has open windows this activates a *Finder window* rather than the bare desktop, and with macOS's "switch to a Space with open windows" setting on, activating Finder can even switch to a Finder window on another space. | macOS exposes no public "focus the empty desktop" API; the moved window would otherwise keep key focus while stashed offscreen. Finder owns the desktop, so activating it is the closest available yield — clean in the common case (Finder with no windows, or windows on the current space, and the space-switch setting off, which is the modern default). | `KiwiCore.desktopFocusYield` (installed in `start()`) calls `NSRunningApplication(bundleIdentifier: "com.apple.finder").activate()`; `moveWindow`'s no-follow branch fires it only when the active space empties and the moved window held key focus ([#446](https://github.com/hajiboy95/KiwiDesk/issues/446)). | Leave Finder without open windows, or turn off "When switching to an application, switch to a Space with open windows"; a window remaining on the space is refocused instead of the desktop. |
