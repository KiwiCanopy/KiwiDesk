...
| In the track layout with `cascade_overflow`, when there are more tracks (or windows in a track) than can hold `min_window_size` even after tiling the fitting prefix, a *buried* multi-window track lands in a thin `min_window_size` slot and its windows then cascade among themselves inside it. | It is the honest answer to "more tracks/windows requested than can hold the minimum at any size": each overflow track keeps a reachable title bar via the shared cascade offset, and the same rule applies one level down inside a buried track — a self-similar nesting, not a broken state. | The overflow layout is the axis-general `OverlapStack.overflowFrames` applied per axis; a cascaded track region is still rendered by the same `trackFrames`, so its own along-axis overflow nests ([#128](https://github.com/hajiboy95/KiwiDesk/issues/128)). | Widen the display, raise the track cap (`track.set_count`), or `cascade_all` for a single uniform pile; the nesting terminates at the whole-region `OverlapStack` fallback. |
