...
| Directional focus or swap issued while a **traveling tiled-sticky window** is frontmost starts from the active space's own last-focused window, not the traveler. | The traveler tiles on the active space but is a member only of its home space, and the per-space `focused` slot is membership-guarded — it can never hold a non-member, so command dispatch falls back to the space's own focus. Navigating *to* the traveler works normally, and a focus-driven layout now surfaces it — Scrolling pans and Monocle raises — via `focusAnchor` ([#431](https://github.com/hajiboy95/KiwiDesk/issues/431)). | One-home membership (`Space.windows` holds a sticky window exactly once) plus `WorkspaceManager.focus`'s membership guard — the same root as the implicit-focused mis-target row above. | Resolve implicit-focused verbs against the OS-focused window id ([#416](https://github.com/hajiboy95/KiwiDesk/issues/416)). |
| Swapping or drag-reordering a traveling tiled-sticky window on a **non-home space** does nothing to its position (a *keyboard* swap partner stays put silently; a bar drag of the traveler is a no-op; a mouse drag — or mouse resize — of the traveler snaps back to its injected slot on drop). The mouse path is *not silent*: the traveler's chip briefly flashes a pill naming its home space — both when the traveler itself is dragged ([#421](https://github.com/hajiboy95/KiwiDesk/issues/421)) and when *another* window is dropped onto the traveler's slot, where the pill lands on the traveler (the window that can't move), not the trier ([#435](https://github.com/hajiboy95/KiwiDesk/issues/435)). Keyboard swaps onto a traveler stay silent for now (no snap-back moment to hang the pill on). | Translating a foreign-space reorder back into a home-array move is the messiest sub-feature of #414 v2 and was cut as a non-goal; `Space.swap`/`move` membership guards make the no-op safe rather than corrupting. Reorder it on its home space — the derived slot follows everywhere. | Injection is derived, never stored: only the home array is real, and every `Space` mutation guards on membership. | A possible future home-array translation ([#414](https://github.com/hajiboy95/KiwiDesk/issues/414) notes it as out of scope for v2). Keyboard-swap-onto-a-traveler is a *semantic* refusal, so it belongs to the pill ([#435](https://github.com/hajiboy95/KiwiDesk/issues/435)), not the dead-end ring rubber-band ([#436](https://github.com/hajiboy95/KiwiDesk/issues/436)) — that cue fires only on a true no-candidate boundary (`.fail("no window … of focus")`), which a found-but-unswappable traveler is not. |
...
its own (it fires no focus event). This is purely a render/raise
anchor: `space.focused` and every implicit-focused verb are
untouched (the *source* of directional focus/swap stays the
space's own slot — see Accepted limitations, and #416). The **App Bar** highlight has the same
