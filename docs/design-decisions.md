...
| A **square** focus border on a *rounded* window overlaps a little more window content near the edges (≈ 0.3× the corner radius) than a rounded border's 1 pt. | Square has no radius to match the window, so a shallow stroke leaves an empty corner between the sharp frame and the window's curve. Tucking the stroke's inner edge to the corner tangent (`R·(1−√2/2)`) makes the stroke band paint over that gap — a uniform inset, so the extra overlap shows on the straight edges too. The alternative (a non-uniform, corner-only inset) would need a custom per-corner path the design deliberately omits. | `BorderGeometry` deepens only the square inner overlap to the tangent point; the rounded path is unchanged, and it stays one stroke routine ([#278](https://github.com/hajiboy95/KiwiDesk/issues/278)). | Pick **Rounded** corners (the default) for the minimal 1 pt overlap; Square trades a little edge coverage for sharp corners with no empty gap. |
