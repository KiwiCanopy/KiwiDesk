...
| In the track layout, when there are more tracks (or windows in a track) than can hold `min_window_size` even after tiling the fitting prefix, a *buried* multi-window track lands in a `min_window_size`-tall cascade slot and its own windows then cascade among themselves inside it. | It is the honest answer to "more tracks/windows requested than can hold the minimum at any size": every buried item keeps a reachable title bar via the shared downward cascade offset (the app-wide title-bar-reveal convention), and the same rule applies one level down inside a buried multi-window track — a self-similar nesting, not a broken state. | The overflow layout is the vertical-cascade authority `OverlapStack.overflowFrames`; a cascaded track region is still rendered by the same `trackFrames`, so its own overflow nests ([#128](https://github.com/hajiboy95/KiwiDesk/issues/128)). | Widen the display or raise the track cap (`track.set_count`); the nesting terminates at the whole-region `OverlapStack` fallback ([#180](https://github.com/hajiboy95/KiwiDesk/issues/180) hard-coded this — the former `cascade_all` knob is gone). |
...
| Turning **Advanced track** off while a space relies on the track cap (`auto_tracks` off + `count`) visibly relayouts: the merged view re-expands to the underlying one-track-per-marker partition. Multi-window tracks built by hand stay untouched. | The cap merge is a read-time *view* over the marker partition, not stored state; the gate clamps the resolved params (cap gone), so the partition simply shows through. Rewriting markers on toggle would destroy state the user may want back when re-enabling. | The gate clamps at resolution (`TrackParams.gated`, after per-space merge) and never mutates `trackBreaks`/`trackWeights` ([#181](https://github.com/hajiboy95/KiwiDesk/issues/181)). | Re-enable Advanced track to restore the capped view; or `move_to_track` the windows back together (the markers are still there). |
| `track.swap` refuses to act while a fixed track limit is folding extra tracks into the last slot (`auto_tracks` off, more marker-tracks than `count`). | The folded view is a read-time merge over the marker partition — its slices have no marker identity, so exchanging them would re-derive a *different* composition after the swap (windows leaking between visible tracks). Rewriting markers to pin the view would destroy the grandfathered partition instead. | The cap merge is position-based (`TrackLayout.counts` folds overflow into the last allowed slot) while reorder ops travel markers with windows ([#182](https://github.com/hajiboy95/KiwiDesk/issues/182) review). | Raise the track limit or turn automatic tracks on, then swap; `move_to_track` still works under the merge. |
...
| **Track** | array-order | steps both axes (`trackStep`) | yes — an overflow cascade (hard-coded, #180; via `OverlapStack.overflowFrames`) |
