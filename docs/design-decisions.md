...
| The searchable app picker's running-app entries are snapshotted when the picker first appears: an app launched *while the Settings window is open* won't show in an already-mounted picker until its view is recreated. Disk-installed apps are unaffected (they're scanned once per process). | The old `Menu` re-queried `NSWorkspace.runningApplications` on every open; freezing the union into view `@State` on `.onAppear` drops that per-render query, and a settings picker rarely needs an app that launched mid-edit. Each picker also snapshots at its own `onAppear`, so two open pickers can differ slightly. | The picker holds `installedApps` (disk apps ∪ running apps) in view-scoped `@State`, loaded once on appear rather than recomputed per render ([#263](https://github.com/hajiboy95/KiwiDesk/issues/263)). | Reopen the Settings window (or the picker's tab) to re-snapshot; disk-installed apps never need this. |
