...
| The App Bar's icon styles offer System default, Tinted, and Glyphs — never the system's **Dark** or **Clear** icon looks, and Tinted is an approximation: a luminance ramp over the flattened icon, not Apple's plate-plus-glyph regeneration (its Light/Dark variants recolor the squircle plate and contrast-match the glyph, which needs the icon's separate layers). | The flattened bitmap is all the public API hands out; ramp inversion (`tint_appearance`) is the nearest bitmap-level equivalent, and painting our own plate under a flattened icon would double the plate. | macOS exposes no public API that hands an app another app's (or even its own) styled icon rendering or its icon layers — Apple DTS calls it unsupported ([#294](https://github.com/hajiboy95/KiwiDesk/issues/294)). | A private-IconServices probe with public fallback, the SkyLight `dlsym` pattern ([#362](https://github.com/hajiboy95/KiwiDesk/issues/362)); if viable the picker grows the true system styles. |
...
**App icon rendering is one global choice with three honest
options.** (#294.) `icon_source` — GUI label "App symbol style" —
offers `app_image` (System default), `tinted_image` (Tinted), and
`app_font` (Glyphs). Decisions folded in, 2026-07-17 (ui-designer
consult + user direction in chat):
...
- **Tinted and Glyphs share one color system**: the bar's state
  text colors (normal / active / hover). No separate tint-color
  knob — a fixed tint was tried mid-design and rejected because it
  decoupled icons from the theme the glyphs follow. Tinted's one
  extra control is `tint_appearance` (Auto | Light | Dark): the
  ramp's brightness, with Auto following the system appearance
  the way the system's own Tinted style does.
- **Glyph-less apps under Glyphs fall back to a tinted icon**, not
  the full-color image, so a glyph bar stays monochrome instead of
  one colorful icon popping out.
- **Dark / Clear system styles are absent by API reality**, not
  choice — see the accepted-limitations row; [#362](
  https://github.com/hajiboy95/KiwiDesk/issues/362) tracks the
  private-IconServices probe that could add them.
...
- **The shortcuts panel follows the bar**: with Glyphs active its
  Apps band leads with the same ligatures, so the two surfaces
  never disagree about an app's symbol.
