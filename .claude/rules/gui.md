...
are BESPOKE views, not `ForEach`es over an order list. Their
order lists exist so the census still records those rows for
the placement table and for search, and the guard holds their
MEMBERSHIP, but editing one moves nothing on screen. Which ones
is data — `ShortcutsRowOrder.bespokeContainers`, the one copy,
asserted by `ShortcutsBespokeContainerTests`, whose table of
order lists is a census held against the declarations in every
`ShortcutsRowOrder` body (#1121), so an order list added without
