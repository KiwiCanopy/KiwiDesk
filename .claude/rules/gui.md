...
- **The accent marks control FILLS, never text naming a value.**
  ...
  **A control style that colours its label from the tint owes a
  ...
  `neutralMenuLabel()`, and since #1502 the menu-style pickers
  pair the same modifier, `.pickerStyle(.menu)` →
  `neutralMenuLabel()`, because macOS 27 draws a picker's label
  from the tint while the window is key where macOS 26 did not
  — a style the census does not count is one the next OS can
  turn green (`SettingsLabelNeutralityTests`) — and a
