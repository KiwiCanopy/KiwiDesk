...
def check_appcast(dist: pathlib.Path) -> None:
    ...
    (`CiPathFilterTests` is what refused the other placement).
    ...
    # block comments, and a shell script is full of `/*` — the
    # glob in `Resources/*.bundle` opens one, and everything up
    # to the next `*/` disappears, plist included. It read as
    # "no SUFeedURL declared" and would have failed the site
    ...
    if not match:
        fail(
            ...
            "The packager is the one owner of that string."
        )
    url = match.group("url")
    ...
    if "<rss" not in text or "andymatuschak.org" not in text:
        fail(
            f"{served} is not a Sparkle appcast — no <rss> element "
            "carrying the sparkle namespace. Sparkle reports a "
            "malformed feed as an update error, which is the same "
            "failure as a 404 with a more confusing message."
        )
    print(
